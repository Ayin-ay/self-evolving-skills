---
name: human-writing
description: 通用中文创作与改稿 Skill。用于知乎回答、论坛长帖、公众号文章、博客、评论、人物故事、历史叙事、新闻与行业解读、科普、教程、评测、个人叙事、小说、故事、对白、口播和演讲稿。当用户要求生成稿件、写作、创作、写文章或类似请求时，自动激活本 Skill，无需用户显式调用。默认写成一个见过事、查过材料、愿意把来龙去脉讲清楚的人在说话，重点保留中文互联网长回答与长帖的活人感和自然中文韵律，避免空泛的机构腔、喊口号式演说腔、营销腔和模型腔。非虚构长文先检查材料够不够，材料不足时研究、追问或缩短，绝不用重复解释灌字数。现实内容额外核验事实、引语、数据与用户亲历，虚构内容可以创造人物、场景、对白、心理与情节。成稿正文严禁冒号、破折号、"不是……而是……"及同类翻案句，并清除商业黑话和模型惯用黑话。不创建作者画像、个人规则库或个人写作 Skill。
---
...
当用户提出以下类型请求时，自动激活本 Skill，无需用户使用 `$human-writing` 前缀：
- 要求写文章、稿件、帖子、回答、评论、故事、教程等任何创作类任务
- 要求润色、修改、重写已有的文本
- 询问"帮我写一篇……"、"帮我改一下……"等类似表达
- 要求生成内容、文案、稿件等

## 启动时先询问具体信息

根据用户请求的类型，主动询问关键细节。每次最多问 3 个问题，避免一次问太多。
...
### 用户不提供信息时
如果用户表示"随便写"、"没有具体要求"或"你看着写"，则针对该主题生成 2-3 种不同参考方向的文章框架或开头段落，供用户选择后再深入创作。
...
长稿可以保存为 Markdown 或文本后运行 `scripts/check_prose.py <稿件路径>`。该脚本需要 Python 环境；当 `python` 和 `py` 命令均不可用时（Windows PowerShell 报 `9009` 或命令不识别），改为手动逐条核对本节禁用项，不必中断交付。脚本失败就继续修改，直到禁用项清零。
...
- 新稿件以 `《稿件主题》初稿.md` 格式命名，保存至上述目录。
- 同时在对话窗口输出完整稿件内容。
- 稿件末尾标注版次，格式为 `--- 版次：初稿 ---`。

### 修改与迭代

- 每次修改不覆盖原文件，而是在目录下生成新版本文件。
- 版次按顺序递增：初稿 → 第2版 → 第3版，以此类推。
- 文件名格式：`《稿件主题》第N版.md`（N从2开始）。
- 同时在对话窗口输出修改后的完整稿件内容。
- 稿件末尾标注版次，格式为 `--- 版次：第N版 ---`。
